...
public class SwingClient {
    ...
    private void clientSetup() {
        ...
        List<entity.Subscriber> list = topicManager.mySubscriptions();
        for (entity.Subscriber e_sub : list) {
            my_subscriptions.put(e_sub.getTopic(), subscriber);
        }
    ...
    class newPublisherHandler implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            // [Local version was buggy here]

            if (!my_subscriptions.containsKey(publisherTopic)) {
                Subscriber subscriber = new SubscriberImpl(SwingClient.this);
                Subscription_check sub_check = topicManager.subscribe(publisherTopic, subscriber);
                if (sub_check.result.equals(Subscription_check.Result.OKAY)) {
                    my_subscriptions.put(publisherTopic, subscriber);
                    if (my_subscriptions_TextArea.getText().trim().equals("")) {
                        my_subscriptions_TextArea.append(publisherTopic.name);
                    } else {
    ...
    class postEventHandler implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            Message message = new Message(publisherTopic, argument_TextField.getText());
